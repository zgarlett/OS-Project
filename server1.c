...
void * serverStart1(void *vargs);
void * listenMessageServer1(void *vargs);
...
int main(){
  ...
  pthread_t thread;
  pthread_create(&thread, NULL, serverStart1, NULL);
...
void* serverStart1(void *vargs){
    ...
    pthread_create(&t2,NULL,listenMessageServer1,NULL);
...
void * acceptConnections(void * vargs){
  ...
  while (clientfd = accept(servSock,NULL,NULL)){
      ...
      mq_send (server2, buffer, strlen (buffer) + 1, 0);
